...
USE `birdbook`$$
DROP TRIGGER IF EXISTS `birdbook`.`USUARIO_AFTER_DELETE_PREF` $$
USE `birdbook`$$
CREATE DEFINER = CURRENT_USER TRIGGER `birdbook`.`USUARIO_AFTER_DELETE_PREF` AFTER DELETE ON `USUARIO` FOR EACH ROW
BEGIN
...
USE `birdbook`$$
DROP TRIGGER IF EXISTS `birdbook`.`PASSARO_AFTER_DELETE_PREF` $$
USE `birdbook`$$
CREATE DEFINER = CURRENT_USER TRIGGER `birdbook`.`PASSARO_AFTER_DELETE_PREF` AFTER DELETE ON `PASSARO` FOR EACH ROW
BEGIN
    DELETE FROM USUARIO_PREFERE_PASSARO
